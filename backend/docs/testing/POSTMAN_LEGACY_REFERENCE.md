# Postman 레거시 참조 (전체 본문)

현행 URL 스냅샷(짧은 표): 같은 폴더의 [POSTMAN_CURRENT_REFERENCE.md](POSTMAN_CURRENT_REFERENCE.md) — 컨트롤러 베이스 경로·자주 쓰는 전체 URL.

도메인 규칙·상태 정본은 `docs/domains/` 및 `docs/architecture/status/status-model.md`를 본다. 요약·링크·최소 시나리오는 [POSTMAN_TEST_GUIDE.md](POSTMAN_TEST_GUIDE.md)에 있다. 이 파일은 기존 한 파일에 모여 있던 긴 절차·요청 예시·번호 섹션을 보존한 것이다.
...
### 유효성/보안
- HMAC 서명 검증 필수: 서버 환경에 payments.toss.secret-key가 없으면 401
...
- Body(JSON)
```
{ "email": "user@example.com", "password": "******", "portal": "CUSTOMER" }
```
- Postman에서 쿠키 자동저장 켜기(세션 유지)
- 권장: 고객몰 테스트는 `portal: "CUSTOMER"`, 관리자/파트너 어드민 테스트는 `portal: "ADMIN"` 을 함께 보냅니다.
- 참고: 브라우저 요청은 `Origin/Referer` 로도 포털을 추론하지만, Postman은 해당 헤더가 없을 수 있으므로 `portal` 명시를 권장합니다.
...
```
- 참고: 필수 아님(현재는 로깅용 스켈레톤)
- 현재 구현은 요청 payload를 로그로 남기고 `{"status":"READY"}`만 반환합니다.
- 주문 존재/금액 검증, 거래 레코드 생성은 아직 TODO 상태입니다.
...
```
- 기대: HTTP 200 + `success=true` 이지만 `data.approved=false`, `data.message`에 금액 불일치 사유 반환, 주문 상태 변화 없음
...
### 변경 핵심
- 프론트 기준 호출 흐름은 2단계입니다.
  1. `GET /api/reviews/partner/ai-candidates` 로 조건에 맞는 후보 리뷰 조회
  2. `POST /api/ai/review-analysis/partner` 로 구조화 분석 요청
- `productNos` 는 현재 단일 상품만 허용합니다.
- `partnerId` 는 요청 바디에 있어도 되지만, 실제로는 세션의 파트너 ID로 덮어씁니다.
- 기간을 지정하지 않으면 전체에서 최신 N건, 기간을 지정하면 그 기간 안에서 최신 N건을 가져옵니다.
- `sentiment`, `confidence`, `actions[].expectedImpact` 필드는 더 이상 사용하지 않습니다.
- 최소 리뷰 수 미만이면 `400 REVIEW_ANALYSIS_NOT_ENOUGH_REVIEWS` 를 반환합니다.
...
### 관련 설정
- `ai.review-analysis.recent-count`: 기본 분석 대상 최신 건수
- `ai.review-analysis.minimum-required`: 분석 가능 최소 리뷰 수
- `ai.review-analysis.hard-cap`: 요청 가능한 최대 건수 상한

### 1) 후보 조회
#### 엔드포인트
```
GET http://localhost:8080/api/reviews/partner/ai-candidates
```

#### Query Params
- `productNo` (필수): 상품 번호
- `optionNos` (선택, 반복 파라미터): 옵션 번호 목록
  - 예: `?productNo=2&optionNos=7&optionNos=8`
- `fromAt` (선택): `2026-04-01T00:00:00`
- `toAt` (선택): `2026-04-08T23:59:59`
- `limit` (선택): 기본은 `recent-count`, 최대 `hard-cap`

#### 동작 규칙
- 기간 미지정: 전체 리뷰 중 최신 N건
- 기간 지정: 해당 기간 안에서 최신 N건
- 옵션 지정: 해당 옵션 리뷰만 대상
    ...
    "reviews": [
      {
        ...
        "productNo": 101,
        "optionNo": 10001,
        "reviewContent": "사이즈가 조금 작아요",
        "reviewRating": 3,
        "reviewCreatedAt": "2026-04-07T10:30:00"
      }
    ],
    "meta": {
      "recentCountDefault": 3,
      "minimumRequired": 3,
      "hardCap": 10,
      "requestedLimit": 3,
      "poolScanned": 1
...
#### 해석
- `recentCountDefault`: 서버 기본 최신 건수
- `minimumRequired`: AI 분석 가능 최소 리뷰 수
- `requestedLimit`: 이번 요청에서 실제 적용된 최대 건수
- `poolScanned`: 현재 조건으로 실제 조회된 후보 수(디버깅 참고)
...
#### 요청 규칙
- `productNos` 는 0개 또는 1개만 허용
- `reviews` 안에 서로 다른 상품 번호가 섞이면 안 됨
- `partnerId` 는 세션 값으로 강제 주입되므로 테스트 편의상 넣어도 실제 값은 무시됨
- `reviews.length < minimumRequired` 면 분석 전에 400 반환
...
```json
{
  ...
  "message": "성공",
  "data": {
    "summary": "사이즈 관련 불만과 배송 지연 언급이 반복되어 안내 문구 보강이 필요합니다.",
    "issues": [
      {
        "type": "DELIVERY_DELAY",
        "frequency": 1,
    ...
    "actions": [
      {
        "area": "SIZE_GUIDE",
        "recommendation": "사이즈 가이드 표와 착용 예시 설명을 보강하세요."
      },
      {
        "area": "CS_MACRO",
        "recommendation": "배송 지연 안내 문구와 상담 답변 템플릿을 정리하세요."
      }
    ],
    "topKeywords": [
      {
...
{
  "success": false,
  "code": "REVIEW_ANALYSIS_NOT_ENOUGH_REVIEWS",
  "message": "리뷰 AI 분석은 최소 3건 이상 필요합니다. (현재 2건)",
  "data": null,
  "errors": null
}
```
...
### 권장 테스트 시나리오
1. 후보 조회 / 최신 모드
   - 기간 없이 `GET /api/reviews/partner/ai-candidates`
   - 기대: 전체에서 최신 `recent-count` 건 반환
2. 후보 조회 / 기간 모드
   - `fromAt`, `toAt` 포함
   - 기대: 해당 기간 안에서 최신 `recent-count` 건 반환
3. 정상 분석
   - 후보 3건 이상으로 `POST /api/ai/review-analysis/partner`
   - 기대: 구조화 응답 반환
4. 리뷰 수 부족
   - `reviews` 를 `minimumRequired` 미만으로 전달
   - 기대: `400`, `code=REVIEW_ANALYSIS_NOT_ENOUGH_REVIEWS`
5. GPT 장애
   - API 키 오입력 / 쿼터 초과
   - 기대: `success=true` 이지만 fallback 응답 (`summary=리뷰 분석 일시 불가`)

## 반품 검토 보조(관리자 전용) 가이드

### 변경 핵심
- OpenAI·사기 점수·`POST /api/ai/return-risk/score`는 제거되었습니다.
- 관리자 전용 **규칙 기반** 검토 보조: `summary`, `checkPoints`, `evidenceStatus`, `signals`, `reviewPriority`.
- 엔드포인트: `GET /api/returns/{returnNo}/return-assist` (하위 호환: `review-assist`, `ai-assist`).

### 권한/보안 정책
- `GET /api/returns/{returnNo}/return-assist` — `ADMIN`만 허용 (`CUSTOMER`, `PARTNER` → `FORBIDDEN`)

### Feature Flag
- 설정: `return.assist.enabled` (환경변수 `RETURN_ASSIST_ENABLED`, 레거시 `RETURN_REVIEW_ASSIST_ENABLED`, `AI_FEATURE_ENABLED` 폴백)
- `false` 시 수동 검토 안내 응답

```properties
RETURN_ASSIST_ENABLED=false
```

### 응답 스키마
`ApiResponse<ReturnAssistResponseDto>` 기준 `data`:

```json
{
  "summary": "상품 불량·하자 사유 반품으로 추가 확인 후 처리하는 편이 안전한 건입니다. 반품 빈도나 동일 주소 반복은 참고 신호로만 보고, 이번 반품의 증빙과 정책 요건을 우선 확인하세요.",
  "reviewPriority": {
    "code": "MEDIUM",
    "label": "추가 확인 권장",
    "reason": "즉시 처리보다는 증빙과 주문 내역을 한 번 더 확인하는 것이 좋습니다."
  },
  "evidenceStatus": {
    "code": "PARTIAL",
    "label": "일부 보완 필요",
    "detail": "사진이 1장만 등록되어 있습니다. 내용이 충분하면 그대로 검토하시고, 필요할 때만 추가 촬영을 요청하세요."
  },
  "checkPoints": [
    "주문 상품과 촬영 상품 일치 여부 확인",
    "추가 촬영 요청 필요 여부 확인",
    "주문 옵션·배송 상태 일치 여부 확인"
  ],
  "signals": {
    "customerSignals": [
      "최근 90일 반품 이력이 있습니다."
...
요청:
```http
GET /api/returns/1001/return-assist
Cookie: JSESSIONID=...
```
...
```bash
cd backend
.\gradlew.bat test --tests "com.swimshop.swim_mall.return_order.service.ReturnServiceReturnAssistTest" --tests "com.swimshop.swim_mall.return_order.service.ReturnCustomerHistoryInsightServiceTest"
```

필수 검증 포인트:
1. customer 접근 차단 (`FORBIDDEN`)
2. admin 접근 허용
3. 응답이 `summary/reviewPriority/evidenceStatus/checkPoints/signals` 구조로 반환되는지 확인
4. 증빙 상태와 신호가 사유/이미지 조건에 맞게 매핑되는지 확인
5. 고객 신호가 메인 판단이 아니라 참고 맥락으로만 노출되는지 확인
6. `return.assist.enabled` / `RETURN_ASSIST_ENABLED` on/off 분기

수동 테스트:
1. 보조 활성 상태에서 관리자 반품 상세 → 검토 보조 패널 표시
2. `RETURN_ASSIST_ENABLED=false` 재기동 후 → 수동 검토 안내
3. 고객 세션으로 `GET /api/returns/{returnNo}/return-assist` → `FORBIDDEN`
...
PUT  http://localhost:8080/api/partner/products/{productNo}
```

### 사이즈 가이드 필드 규칙 (신규)
- `sizeGuideJson`: 구조화 사이즈표(JSON 문자열, 신규 권장)
- `sizeGuideText`: 자유 텍스트 사이즈 안내(구형/비구조 상품 fallback)
- 구조화 사이즈표를 쓰는 경우:
  - `sizeGuideJson` 사용
  - `sizeGuideText`는 `null` 권장

### `sizeGuideJson` 템플릿 규칙
- `productType=SWIMSUIT_MEN` -> `templateKey=SWIMSUIT_MEN_PANTS`
  - row 컬럼: `sizeLabel`, `waistCm`, `hipCm`
- `productType=SWIMSUIT_WOMEN`, `productSubType=BIKINI` -> `templateKey=SWIMSUIT_WOMEN_BIKINI`
  - row 컬럼: `sizeLabel`, `chestCm`, `waistCm`, `hipCm`
- `productType=SWIMSUIT_WOMEN`, 그 외 서브타입 -> `templateKey=SWIMSUIT_WOMEN_ONEPIECE`
  - row 컬럼: `sizeLabel`, `chestCm`, `waistCm`, `hipCm`, `torsoCm`
- `productType=FINS` -> `templateKey=FINS_SIZE`
  - row 컬럼: `sizeLabel`, `footLengthCm`

### 등록 요청 예시 (남성 수영복 바지)
```json
{
  "productName": "남성 잠머 테스트",
  "productType": "SWIMSUIT_MEN",
  "productSubType": "JAMMER",
  "productPrice": "89000",
  "productDescription": "남성 수영복 바지 테스트 상품",
  "productImageUrl": "/images/products/test-jammer.jpg",
  "materialInfo": "폴리에스터 78%, 스판덱스 22%",
  "originCountry": "대한민국",
  "manufactureCountry": "대한민국",
  "careInstructions": "30도 이하 단독 손세탁 권장",
  "sizeGuideText": null,
  "sizeGuideJson": "{\"templateKey\":\"SWIMSUIT_MEN_PANTS\",\"rows\":[{\"sizeLabel\":\"M\",\"waistCm\":78,\"hipCm\":95},{\"sizeLabel\":\"L\",\"waistCm\":82,\"hipCm\":99}]}",
  "options": [
    { "color": "블랙", "size": "M", "optionAddPrice": 0 },
    { "color": "블랙", "size": "L", "optionAddPrice": 3000 }
  ]
}
```
...
```json
{
  "productName": "숏핀 테스트",
  "productType": "FINS",
  "productSubType": "FINS_SHORT",
  "productPrice": "65000",
  "productDescription": "숏핀 테스트 상품",
  "productImageUrl": "/images/products/test-fins.jpg",
  "sizeGuideText": null,
  "sizeGuideJson": "{\"templateKey\":\"FINS_SIZE\",\"rows\":[{\"sizeLabel\":\"250\",\"footLengthCm\":25.0},{\"sizeLabel\":\"255\",\"footLengthCm\":25.5}]}",
  "options": [
    { "color": "블랙", "size": "250", "optionAddPrice": 0 },
    { "color": "블랙", "size": "255", "optionAddPrice": 0 }
  ]
}
```
...
```json
{
  "productName": "여성 비키니 수정본",
  "productType": "SWIMSUIT_WOMEN",
  "productSubType": "BIKINI",
  "productPrice": "99000",
  "productDescription": "여성 비키니 수정 테스트",
  "productImageUrl": "/images/products/test-bikini.jpg",
  "sizeGuideText": null,
  "sizeGuideJson": "{\"templateKey\":\"SWIMSUIT_WOMEN_BIKINI\",\"rows\":[{\"sizeLabel\":\"S\",\"chestCm\":82,\"waistCm\":64,\"hipCm\":88},{\"sizeLabel\":\"M\",\"chestCm\":86,\"waistCm\":68,\"hipCm\":92}]}",
  "options": [
    { "optionNo": 101, "color": "블랙", "size": "S", "optionAddPrice": 0 },
    { "optionNo": 102, "color": "블랙", "size": "M", "optionAddPrice": 0 }
  ]
}
```

### 실패 케이스 예시
- `templateKey`가 상품 유형과 불일치
  - 예: `productType=SWIMSUIT_MEN`인데 `templateKey=SWIMSUIT_WOMEN_ONEPIECE`
- `rows`가 비어 있음
- `sizeLabel` 중복
- 수치 컬럼에 0 이하 또는 숫자 아님

### 참고
- 현재 백엔드는 `sizeGuideJson`을 우선 사용하며, `sizeGuideText`는 기존 데이터 호환용으로 유지합니다.
- 구조화 사이즈표 대상 타입(`SWIMSUIT_MEN`, `SWIMSUIT_WOMEN`, `FINS`)에서는 `sizeGuideJson` 사용을 권장합니다.
...
- `size`: 페이지 크기 (기본값 `12`)
- `saleOnly`: 세일 상품만 조회 여부 (`true|false`)
- `inStock`: 재고 있는 상품만 조회 여부 (`true|false`)
- `sortBy`: 정렬 기준 (`latest` 등)
...
1. 고객 로그인
   ```
   POST http://localhost:8080/api/auth/login
   Body: { "email": "customer1@swim-mall.com", "password": "customer123", "portal": "CUSTOMER" }
   ```
...
#### 현재 백엔드 규칙
- 구매확정 기본 포인트
  - 트리거: `POST /api/orders/order-items/{orderItemNo}/complete` 또는 자동 구매확정 스케줄러
...
2. `POST /api/orders/order-items/{orderItemNo}/complete` 호출
3. 확인
...
1. 관리자 이벤트 생성
   - `eventType=POINT`
   - `eventMode=ADMIN_ONLY`
   - `eventStatus=PUBLISHED` (포인트 적용 대상 조회 시)
...
#### 이벤트 상태 (`eventStatus`) — 3종 (API 문자열 그대로 사용)

| 값 | 의미 |
|-----|------|
| `PRIVATE` | 비공개 · 고객·파트너 공개 목록 미노출 (과거 `DRAFT`, `INACTIVE`) |
| `PUBLISHED` | 공개 · 목록 노출 허용. 고객 기간 안에서만 `visibleToCustomerNow=true` |
| `ENDED` | 종료 · 불가역 전환 |

- 고객/파트너 목록 API에는 `PUBLISHED` 또는 `ENDED` 행만 포함되며 `PRIVATE`는 미포함입니다.
- 기간 종료 후 스케줄러가 `PUBLISHED` → `ENDED` 로 자동 전환합니다.

#### 참여 형태 (`eventMode`) 및 타입 (`eventType`) 호환 규칙

- `eventType=SALE` ⇒ `eventMode`는 반드시 `PARTNER_PARTICIPATION` (파트너 신청·등록).
- `eventType=NOTICE` ⇒ `eventMode`는 반드시 `ADMIN_ONLY` (운영 직접 등록).
- `eventType=POINT` ⇒ `ADMIN_ONLY` 또는 `PARTNER_PARTICIPATION` 허용.

(`PARTNER_PARTICIPATION` 선택 시 요청 바디에서 `partnerApplyEnabled`는 서버가 `eventMode`에 맞춰 처리합니다.)
...
(`PATCH …/status` 전이: 비공개↔공개 왕복 가능, 종료만 예외 처리. 종료 후에는 변경 불가.)
...
주의: `GET /api/partner/events/{eventNo}/participation` 참여 여부 조회 엔드포인트는 현재 제거되었습니다.  
참여 여부는 `GET /api/partner/events` 목록 응답의 `participationEnabled`, `participating` 필드로 확인합니다.
...
설명: 토스 결제 위젯 성공 페이지에서 받은 `orderId`, `paymentKey`, `amount`로 결제를 승인합니다. 성공 시 주문 상태는 `PENDING_PAYMENT` → `PAID`로 변경됩니다.

⚠️ 중요:
- 현재 공개 승인 엔드포인트는 `/api/payments/confirm`만 사용합니다.
- 이 API는 세션 없이도 호출 가능합니다.
- 업무 실패(금액 불일치, 주문 없음, 상태 불일치 등)는 `success=false`가 아니라 `success=true` + `data.approved=false` 형태로 반환될 수 있습니다.
...
```json
{
  ...
  "orderId": "ORD-1-1710000000000",
  "amount": 150000
}
```

요청 파라미터:
- `paymentKey` (필수): 토스 결제 키
- `orderId` (필수): 위젯 orderId. 일반적으로 `ORD-{orderNo}-{suffix}` 형식
- `amount` (필수): 결제 금액
...
엔드포인트: 공개 HTTP API 없음

설명: 현재 백엔드에는 `POST /api/payments/{orderNo}/fail` 같은 공개 실패 처리 엔드포인트가 없습니다.  
결제 실패는 아래 흐름에서 내부적으로만 반영됩니다.
- `POST /api/payments/confirm` 호출 중 토스 승인 실패
- `POST /api/payments/webhook` 로 수신된 취소/실패 계열 이벤트

### 결제 API 주의사항

1. 주문 상태 전이:
   - 결제 승인 성공: `PENDING_PAYMENT` → `PAID`
   - 이후 발주 확인이 완료되면 주문 상태가 `ACTIVE`로 전이됩니다.

2. 권한 검증:
   - `POST /api/payments/confirm` 자체는 세션 기반 권한 검증을 하지 않습니다.

3. 중복 결제 방지 (강화):
   - `PENDING_PAYMENT` 상태인 주문만 결제 승인/실패가 가능합니다.
   - 이미 결제 성공 레코드가 있으면 `approved=true`, `idempotent=true`로 반환될 수 있습니다.

4. 금액 검증:
   - `amount`는 필수이며 서버의 주문 금액(`orderTotalPrice`)과 비교 검증합니다.
   - 금액이 일치하지 않으면 HTTP 200 응답 안에 `approved=false`로 반환됩니다.

5. 테스트 모드:
   - 토스 시크릿 키(`payments.toss.secret-key`)가 없으면 `approved=false`로 반환됩니다.
...
```
예상 결과: `approved=true`, 주문 상태 `PAID`
...
```
예상 결과: 이미 결제 성공 이력이 있으면 `approved=true`, `idempotent=true`
...
설명: 현재 로그인한 고객의 주문에 연결된 결제 정보를 조회합니다.
...
주의: 현재 공개 API에는 `GET /api/payments/{paymentNo}` 엔드포인트가 없습니다.
...
설명: `6.1`과 동일한 엔드포인트 설명입니다. 현재 로그인한 고객의 주문에 연결된 결제 정보를 조회합니다.
...
- 서버 페이지네이션과 서버 상태 필터를 지원합니다.
- 응답에는 현재 페이지의 `items`와 함께 탭 표시용 전체 집계 `statusCounts`가 포함됩니다.
...
- `size`: 페이지 크기 (기본값 `10`)
- `status`: 상태 필터
  - 기본 주문 상태: `PENDING_PAYMENT`, `PAID`, `ACTIVE`, `PAYMENT_FAILED`, `CANCELLED`
  - 관리자 화면 전용 파생 필터: `PENDING_CONFIRMATION`, `DELIVERY_DELAY`
  - 미지정 시 전체 조회

요청 예시:
```http
GET /api/orders/admin?page=1&size=10&status=PAID
```
    ...
    "items": [
      {
        ...
        "orderCreatedAt": "2024-01-15T10:30:00",
        "orderStatus": "PAID",
        "orderDisplayStatus": "PENDING_CONFIRMATION",
        "orderDisplayStatusLabel": "발주 확인 대기",
        "recipientName": "홍길동",
        "recipientPhone": "010-1234-5678",
        "deliveryAddress": "서울시 강남구 테헤란로 123",
        "deliveryAddressDetail": "456호",
...
응답 필드 설명:
- `items`: 현재 페이지에 해당하는 주문 목록
- `total`: 현재 필터 기준 전체 주문 수
- `page`: 현재 페이지 번호
- `size`: 현재 페이지 크기
- `statusCounts`: 전체 결과 기준 상태별 건수
  - `ALL`: 전체 주문 수
  - `PENDING_CONFIRMATION`: 발주 확인이 필요한 주문 수
  - `DELIVERY_DELAY`: 출고 지연 의심 주문 수

권한/동작 참고:
- 파트너 응답 구조도 동일하지만 `items`와 `statusCounts` 모두 자신의 상품 주문 기준으로만 계산됩니다.
- `statusCounts`는 현재 페이지 기준이 아니라 전체 결과 기준입니다.
...
설명: 엔드포인트는 남아 있지만, 현재 구현에서는 수동 주문 상태 변경을 지원하지 않습니다. 항상 `403 FORBIDDEN`을 반환하며 결제/발주 확인/배송/반품 전용 API를 사용해야 합니다.
...
현재 응답 예시 (403 Forbidden):
```json
{
  "success": false,
  "code": "FORBIDDEN",
  "message": "주문 상태 수동 변경은 제공하지 않습니다. 결제·발주 확인·배송·반품 처리 흐름을 이용해 주세요."
}
```
...
1. 권한별 접근 범위:
   - 관리자(ADMIN): 모든 주문 조회 가능, 발주 확인 가능
   - 파트너(PARTNER): 자신의 상품 주문만 조회 가능, 발주 확인 가능
...
2. 발주 확인 vs 주문 상태 변경:
   ...
   - 주문 상태 변경 (`PATCH /api/orders/{orderNo}/status`): 현재 비활성화됨. 항상 403 반환

3. 주문 상태 변경 제한:
   - 수동 상태 변경은 현재 제공하지 않음
   - 상태는 결제 승인, 발주 확인, 배송, 반품 흐름에서만 변경
...
설명: 배송 송장번호와 택배사 정보를 수정합니다. 현재 구현에서는 `DELIVERED`가 아닌 배송이면 수정할 수 있습니다. 즉 `READY`, `SHIPPED` 상태에서 수정 가능합니다.
...
- `imageFileIds` (조건부): `/api/files/upload` (`category=return-image`)로 업로드한 파일 ID 목록(최대 3장). `DEFECT`·`WRONG_ITEM`·`OTHER`일 때 최소 1장 필수. `CHANGE_OF_MIND`·`ORDER_MISTAKE`는 생략 가능(`[]` 전송).
...
- 환불 완료(`REFUNDED`)된 주문 상품은 리뷰 작성 불가
...
주의: 관리자 세션이면 전체 리뷰를, 파트너 세션이면 자신의 상품 리뷰만 조회합니다.
...
5. 리뷰 조회:
   ...
   - 파트너 세션으로 `/api/reviews` 호출 시 자신의 상품 리뷰만 조회
...
주의: 문서상 권장 사용자는 파트너이지만, 현재 구현상 이 경로는 admin 세션으로도 호출 가능합니다. admin 세션이면 관리자/파트너 공용 리뷰 목록 로직을 재사용하고, partner 세션이면 자신의 상품 리뷰만 반환합니다.
...
```json
{
  ...
  "businessRegistrationFileId": 101,
  "bankAccountFileId": 202
}
```
...
- `businessRegistrationFileId`: 업로드된 사업자등록증 파일 ID
- `bankAccountFileId`: 업로드된 통장 사본 파일 ID
...
주의: 관리자만 접근 가능 (현재 권한 체크가 실제 적용됨)
...
- 비활성화된 파트너는 로그인 불가
- 비활성화 시 해당 파트너의 모든 상품과 옵션이 자동으로 INACTIVE 상태로 변경되어 고객 목록에 표시되지 않음
...
#### 엔드포인트
```
GET http://localhost:8080/api/admin/partners?status=APPROVED&hasDeactivationRequest=true
```
...
주의: 관리자만 접근 가능. 현재 구현은 전용 `deactivation-requests` 경로가 아니라 파트너 목록 API에 필터를 붙여 조회합니다.
...
주의: 현재 구현상 `INACTIVE` 계정은 새 로그인 시 `ACCOUNT_INACTIVE`로 차단됩니다. 따라서 일반적인 Postman 로그인 후 호출 흐름으로는 재현이 어렵고, 기존 파트너 세션이 유지된 상황에서만 호출 가능합니다.
...
- 현재 구현상 `INACTIVE` 계정은 새 로그인 시 `ACCOUNT_INACTIVE`로 차단됨
...
- 따라서 일반적인 Postman 로그인 플로우로는 이 API를 바로 재현하기 어려움
...
4. 비활성화 프로세스:
   ...
   - INACTIVE 상태에서는 새 로그인이 차단됨 (`ACCOUNT_INACTIVE`)
   - 비활성화된 파트너는 재활성화 가능
...
12. 재활성화 신청 프로세스:
    ...
    - 다만 현재 구현상 INACTIVE 계정은 새 로그인 시 차단되므로, 기존 세션이 남아 있는 경우에만 직접 호출 가능
    - 한 번에 하나의 재활성화 신청만 가능 (이미 신청한 경우 재신청 불가)
    - 관리자가 승인하면 APPROVED 상태로 변경되고 상품/옵션도 활성화됨
    - 관리자가 거절하면 INACTIVE 상태 유지 및 신청 정보 초기화

13. INACTIVE 상태에서의 기능 제한:
    - 새 로그인 자체가 차단되므로 일반적인 Postman 세션 진입이 불가능
    - 재활성화 신청 API는 기존 세션이 남아 있는 특수한 경우에만 직접 테스트 가능
    - 일반 운영 관점에서는 관리자 승인 API(`PATCH /api/admin/partners/{partnerId}/activate`)를 함께 확인하는 편이 현실적
